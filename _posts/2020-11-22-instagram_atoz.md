...
기본적으로 인스타그램은 비공개 계정으로 설정 시 팔로우한 사람만 게시물과 스토리를 볼 수 있는데요. **팔로우한 사람 중 일부에게만 스토리를 보이도록 올리고 싶거나, 반대로 특정 계정만 스토리가 보이지 않도록 설정** 할 수도 있습니다. 전자의 경우 스토리 업로드 시 친한 친구 그룹에 계정을 추가하고 올리면 되고, 후자의 경우 계정 프로필에서 (…) 아이콘을 누르고 [소식 숨기기 > 스토리]를 체크하면 된답니다. <br>
...
인스타그램에서 기본적으로 필터링 되는 댓글 외 스팸이나 광고 계정의 댓글을 보고싶지 않다면? 인스타그램 우측 상단에 있는 햄버거 (三) 아이콘을 누르고 [설정 > 공개 범위 > 댓글]에서 가장 많이 신고된 단어를 필터링 설정할 수도 있고, 수동 필터링으로 차단하고 싶은 문구를 남겨주세요. 단, 키워드만 등록할 경우 제대로 필터링이 되지 않으니 다음과 같이 문장에 일부를 정확하게 등록해야 된답니다. (ex. 불만 X / 불만이 O) <br><br>
...
올해 직면한 코로나19 장기화로 인해 외출할 때 가장 많이 사용하는 QR코드가 익숙해지면서 점차 활용하는 비중이 늘어나게 되지 않았나 추측됩니다. 네임태그는 캡쳐화면을 바로 스캔할 수는 없기 때문에, 2명 이상일 때 1명은 네임태그 화면을 띄우고 다른 1명은 QR코드를 스캔해야 하는 점 참고하세요! <br><br>
...
인스타그램을 보다 보면, 다른 계정에 업로드 된 게시물을 캡쳐가 아닌 이미지나 영상을 그대로 다운로드 받고 싶은 순간이 있죠! 저는 업무 특성상 PC로 인스타그램을 보는 경우가 많고 레퍼런스 수집을 위해 사용하지만, 덕질을 하는 사람들에게 아주 유용한 기능이 아닐까 해서 소개 드립니다. <br>

바로, 'Download for Instagram'이라는 크롬 확장 프로그램인데요. 크롬 브라우저에서 해당 프로그램을 추가하고 인스타그램을 켜면 PC에서 APP처럼 모바일 UI로 인스타그램을 볼 수도 있고, 게시물과 스토리 업로드는 물론 (단, 태그나 필터 등 일부 기능 사용 불가) 게시물 다운로드까지 가능 하답니다. **다운로드 받은 게시물은 갠소 용으로 간직하고, 저작권을 꼭 지켜주세요!** <br>
